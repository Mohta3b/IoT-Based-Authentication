...
void Application::showHistoryWindow()
{
    ...
    if (_webSocket.state() == QAbstractSocket::ConnectedState) {
        _webSocket.sendTextMessage(QStringLiteral("{\"command\": \"fetch_history\"}"));
    } else {
...
void Application::processMessageReceived(const QString &message) {
    ...
    if (!doc.isNull() && doc.isObject()) {
        ...
        } else if (jsonObject.contains("username") && jsonObject.contains("date") && jsonObject.contains("time")) {

            QString username = jsonObject["username"].toString();
            QString date = jsonObject["date"].toString();
            QString time = jsonObject["time"].toString();
            _window->updateUserDetails(username, date, time);
