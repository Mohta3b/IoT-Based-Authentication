...
void MainWindow::setupConnections()
{
    connect(_connectBtn, &QPushButton::clicked, [this] () {
        emit connectBtnClicked(_addressInput->text(), _usernameInput->text(), _passwordInput->text());
    });
...
void MainWindow::updateUserDetails(const QString &username, const QString &date, const QString &time) {
    _username->setText(QString("شناسه کاربری: %1").arg(username));
    _date->setText(QString("تاریخ: %1").arg(date));
    _time->setText(QString("ساعت: %1").arg(time));
}
